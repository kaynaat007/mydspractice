...
.zip() is used to print columns of matrix
'''
...
def check_row(mat, i, n):
    ...
    count = Counter(row)
    print(count)
    return len(list(filter(lambda x: x > 1, count.values()))) >= 1

def check_col(mat, i, n):
    col  = []
    for j in range(n):
        e = mat[j][i]
        col.append(e)
...
def checkValidUtil(matrix: List[List[int]]) -> bool:
    n = len(matrix[0])
    for i in range(n):
        if check_row(matrix, i, n):
            return False
    for i in range(n):
        if check_row(matrix, i, n):
            return False
...
mat = [[15,7,18,11,19,10,14,16,8,2,3,6,5,1,17,12,9,4,13],
...
# print(checkValidUtil(mat))

print(checkValidUtil2(mat))
